...
- 当前 frozen execution 为 commit `3170d89cfd6066ba494170826cd43626d83c6789`、tree `6bee7c4db4c9adde0612aa7c67467a331d20263e`，state 为 sequence 6 / `InstallStarted`，WAL 4；只读认证得到 current pre-recovery external anchors latest/previous generation 11/10。generation 14/13 是未来 reconciler 成功提交 recovery 后的 postcondition，不得描述为当前状态。recovery 为 3 directories/8 files，Release 为 22 files；Program Files 安装目录存在且为空，ProgramData product root 不存在。
