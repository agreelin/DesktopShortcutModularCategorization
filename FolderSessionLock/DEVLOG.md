...
## 2026-07-31 — CP10 Attempt008 release fingerprint authority repair

- From clean commit `80f92bbecf44f4162567c0ddab67d0074353d224`, tree
  `ed0327d51159137940c6283a68a1527ec87cc725`, formal preparation created
  immutable RAB-7 `install-wal-rollback-7` with contract
  `FSL-CP10-INSTALL-WAL-ROLLBACK-RECOVERY-7`, followed by FLB-8
  `install-wal-rollback-launch-observer-8` with contract
  `FSL-CP10-INSTALL-WAL-ROLLBACK-LAUNCH-OBSERVER-8` for
  `CP10-IWRR-LAUNCH-ATTEMPT-008`. Both public validators returned valid with
  no errors.
- The RAB canonical SHA-256 was
  `9AA5EA45A1D677C42FAB0DDCD1E7DAB82311446AED1CB5B40E4E568762002505`;
  its wrapper and contract file SHA-256 values were
  `1A8A61CB7D7FD10853237349B0653907FA43D206C062960D31A36E4512161209`
  and `F8A2DD8D1C30DFAD7186CAA902311EF5BCFCA8ED39159068779FB8E59FF58817`.
  The FLB canonical SHA-256 was
  `0EB2AD9B8C62D3D8FEC70B99F69EB7BBCA1BE95DD231BA2435578B36EACCF99E`;
  its outer, observer, and contract file SHA-256 values were
  `19121D260386E833941B53CDF8754F589911486EF96AF9FA07EFA86537D5348E`,
  `F38011CF9267B6214DAE4EF3488481F69713583B788A9E1D8DA133878694F302`,
  and `57EE78670A58A07BB6BE396E80EE240EACF94FED34AA2FF35F3FDE38DEE59C60`.
- Independent root verification executed only the actual generated observer
  definitions and the complete `Assert-FormalPreLatch` diagnostic. It did not
  execute the observer top level, outer launcher, latch creation, or RunAs.
  All previously repaired gates passed, but the release gate failed closed
  with exit 74 / `Release fingerprint drifted.` The Attempt008 outer was never
  invoked and `launch-attempt.jsonl` was never created.
- The failure was deterministic. RAB freezes the 22 release files as ordered
  `{path,length,sha256}` records and computes
  `Get-FslRabObjectHash`: `ConvertTo-Json -Depth 64`, CRLF-to-LF
  normalization, removal of trailing CR/LF, exactly one final LF, strict UTF-8
  without BOM, and uppercase SHA-256. Its frozen fingerprint was
  `F5ECE1435052A83B91DB241D5F562BE4059BD8782F884B49449963FC5A146C16`.
  The old emitted observer instead hashed newline-joined
  `Name|Length|Hash` text and produced
  `B5244B4595BB0DC10F9F8B5413BB3829149AF673E5D20B24B2C2B581ABAAA90E`.
  The public FLB validator checked generated bytes and bindings but did not
  execute this mismatched runtime calculation.
- RED reproduced the generated helper failure. The repair emits
  `Assert-ReleaseFingerprint`, validates the exact non-directory file set,
  preserves the contract's frozen record order, distinguishes exact path,
  length, and same-length content-hash drift, rebuilds exact ordered
  `{path,length,sha256}` records, and applies the RAB canonical JSON/hash rule.
  Exact-set, path, length, content-hash, and final fingerprint failures all
  remain fail-closed at exit 74 with distinct diagnostic reasons.
- The FLB regression suite executes the actual generated definitions-only
  helper against the private RAB `Get-FslRabObjectHash` oracle and aggregates
  baseline, missing/extra set, order/casing, length, same-length hash, and
  fingerprint mutations. It passed
  `STAGE4_FORMAL_LAUNCHER_BUNDLE_PASS Cases=248 Assertions=318`.
  The RAB suite's formal read-only probe was also corrected to compare the
  existing formal base root and its exact ordinal immediate-directory
  path/name/identity snapshot before and after the probe instead of asserting
  the obsolete count three; all 15 retained RAB/FLB evidence directories
  remained unchanged. It passed
  `STAGE4_RECOVERY_AUTHORITY_BUNDLE_PASS Cases=222 Assertions=309`.
- Root verification ran both focused C# bridge methods in one command:
  passed 2, failed 0, skipped 0, total 2. Release build completed with
  0 warnings and 0 errors. All three modified PowerShell files parsed with
  0 errors; `dotnet format --verify-no-changes`, `git diff --check`, and the
  exact three-file repair boundary passed.
- The repair was committed as
  `1a2f37ee827e32a28940773d96ea61615e51263c`, tree
  `af6c93bcc4e27c70762bc58108c9a65aff8e959c`. The five RAB-7/FLB-8 artifact
  file hashes above remained unchanged. The bound `stage4-state.json`,
  `stage4-journal.jsonl`, and `install-wal.jsonl` SHA-256 values remained
  `553113C430F645EB911F4BDA3E8FF38FD27D541E0177645A123D20F5CEE17270`,
  `FD1376C84C0588D0D9971F95719138DD570577F79883E7B143DC321F0D9397E8`,
  and `C299D1FE85E542603BABF5DB4B38796343CD8158817853F9E1D53EEFD15CEF69`.
  State sequence 6 / `InstallStarted`, WAL count 4, and authenticated current
  anchor generations 11/10 remained unchanged.
- Final residue checks found zero relevant TEMP entries, product/test
  processes, and recovery services; the Program Files installation directory
  had zero children, ProgramData remained absent, and the Attempt008 latch
  remained absent. No UAC, RunAs, reconciler, WAL, state, journal, anchor,
  installation, ACL, SCM, LocalSystem, restart, logoff, or other system
  mutation occurred.
- Independent reviewer returned `PASS`,
  `BLOCKER/HIGH/MEDIUM/LOW = 0/0/0/0`, with no findings. RAB-7, FLB-8, and
  Attempt008 are retained only as failure evidence and are permanently
  non-reusable after the recovery-toolchain commit changed. This documentation
  freeze does not generate any new formal artifact.
- After the new documentation commit-freeze, the next formal action must
  generate and independently validate RAB-8 `install-wal-rollback-8` with
  contract `FSL-CP10-INSTALL-WAL-ROLLBACK-RECOVERY-8`, followed by FLB-9
  `install-wal-rollback-launch-observer-9` with contract
  `FSL-CP10-INSTALL-WAL-ROLLBACK-LAUNCH-OBSERVER-9`. The only next attempt is
  `CP10-IWRR-LAUNCH-ATTEMPT-009`. UAC execution, WAL records 5–7, anchor
  generations 14/13, installation completion, restart or logoff, D-026
  schema-v2 evidence, unsigned Release delivery, cleanup, and final Stage 4
  remain incomplete.
