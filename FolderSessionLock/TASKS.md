...
- [ ] 阶段 4：提升 Broker、IPC 与恢复生命周期。
  ...
  - [ ] `FSL-STAGE4-VM` 当前单一管理员账户的同账户 UAC、SCM、LocalSystem、recovery/readiness/ACL、必要重启或注销、unsigned Release 与 `D-026` schema v2 证据完成。
    ...
    - [ ] 在新的文档 commit-freeze 后生成并验证 RAB-7 `install-wal-rollback-7` / contract `FSL-CP10-INSTALL-WAL-ROLLBACK-RECOVERY-7`，以及 FLB-8 `install-wal-rollback-launch-observer-8` / contract `FSL-CP10-INSTALL-WAL-ROLLBACK-LAUNCH-OBSERVER-8`；新的唯一 attempt 为 `CP10-IWRR-LAUNCH-ATTEMPT-008`。
    - [ ] 仅在 RAB-7、FLB-8 和完整 pre-latch 重新通过后，使用全新 Attempt008 执行唯一 one-shot observer/UAC；不得 retry、fallback、第二次 Install 或复用旧 latch/attempt。
