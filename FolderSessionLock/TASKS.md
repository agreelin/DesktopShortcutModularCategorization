...
- [ ] 阶段 4：提升 Broker、IPC 与恢复生命周期。
  ...
  - [ ] `FSL-STAGE4-VM` 当前单一管理员账户的同账户 UAC、SCM、LocalSystem、recovery/readiness/ACL、必要重启或注销、unsigned Release 与 `D-026` schema v2 证据完成。
    ...
    - [x] 已从 commit `b2bd99c2ca94e512af5ac57d733eddc4075a0202` 生成并通过 public validator 验证 RAB-4 `install-wal-rollback-4` 与 FLB-5/Attempt005 `install-wal-rollback-launch-observer-5`。RAB canonical SHA-256 为 `10A4E34FF4791330AE978F4A07E838AB07210CE76F8334CD97EC6EB424266483`；FLB canonical SHA-256 为 `6C67F8EE8FDC34EB9238F8E3BCCB280FE3884D2D12FB45CF75A39226977552A2`。
    - [x] Attempt005 outer 从未执行且 latch 从未创建：完整 actual generated `Assert-FormalPreLatch` runtime diagnostic 在任何 top-level/latch/RunAs 前以 exit 70 / `Opaque recovery bindings drifted.` fail-closed。唯一漂移字段为 opaque RAB gate-map authority SHA `54814C482F0EE54AC4112826895ADBBE830D58963AE916FD0435359BEE512D5B` 与错误写入 FLB manifest 的 actual map SHA `AA64692C7AA921784EA074E5859ADC903CDB1B781CEC0C0AD63820B5C91BC1E9`。
    - [x] Attempt005 pre-latch 根因已修复并通过 reviewer：FLB contract builder 混用了 actual gate-map 内容哈希与 RAB binding-manifest authority 哈希，而 observer 会分别验证两种语义。commit `f9df12b51274fac5e41dae82f4f2bef4fa7c3393` 从已做 length/SHA/strict-UTF-8 绑定的 RAB contract 保留 opaque authority hash，同时继续独立绑定 actual map hash；新增跨模块回归测试。FLB 243/313、focused bridge 1/1、parser、format、diff 与残留检查通过；reviewer `PASS`，`BLOCKER/HIGH/MEDIUM/LOW = 0/0/0/0`。
    - [x] 已从 commit `60b3cb4dc5f6477eb756d7c433340a949204fd3b` 生成并通过 public validator 验证 RAB-5 `install-wal-rollback-5` 与 FLB-6/Attempt006 `install-wal-rollback-launch-observer-6`。RAB canonical SHA-256 为 `6F30FF6A2602C3547C664C9352E94A4C1AD66C62A3F9AD7D962FBB1CB1DE2482`；FLB canonical SHA-256 为 `BC23307ADB80784C263175E5F8E7077D42A74F9923722F1BF162EAB222A84928`。
    - [x] Attempt006 outer 从未执行且 latch 从未创建：完整 actual generated pre-latch runtime diagnostic 在 recovery gate 1 前触发 StrictMode `PropertyNotFoundStrict`。Windows PowerShell 5.1 将 `$gates[$i].PSObject.Properties[0].Name` 解析为名称为 `0` 的 property 查询而非数组索引，随后读取 null `.Name` 终止。
    - [x] Attempt006 pre-latch 根因已修复并通过 reviewer：commit `90705bc4587229b100a3dfdc689d3600d94469fe` 先将 property collection 物化为 `$gateProperties` 数组，再按 `[0]`/`[1]` 验证 exact-two 名称；其余 gate count/order/type/ID/exit/duplicate/hash 合同不变。回归测试执行 actual generated gate-loop 片段；FLB 243/313、focused bridge 1/1、parser、format、diff 与残留检查通过；reviewer `PASS`，`BLOCKER/HIGH/MEDIUM/LOW = 0/0/0/0`。
    - [x] 已冻结 RAB-6 `install-wal-rollback-6` 与 FLB-7/Attempt007 `install-wal-rollback-launch-observer-7`：RAB canonical SHA-256 为 `0AD74E42E42904F56DBF5B3347979BF879BE724E4B0272E81E5CCB3CF772E40F`，wrapper SHA-256 为 `CC0A2EF28889ABF575BC80C6331633E07C1D41BFE63311BD426FE87DBBDB19A3`，contract file SHA-256 为 `44B25FC6948D64B9A9AEF816F5F942B917D09BC23496A1BCC6FF8490E3BC5261`；FLB canonical SHA-256 为 `33DB3BC00DFE277FC09EBB5357AC40FBED6F0B857BDDBDB3469CC8A34E8A139B`，outer/observer/contract file SHA-256 依次为 `64FFABD506D973EB1E4E9A086A52916D221F7EE5EAC80316444515E082D9BDA4`、`E043B5973CE99CA822F95EBCBA4ACCC40077F7A253F52F16A78B787B02139C90`、`DBCA7E9934C6B6A7EC6FE8980D0AED9209CF7A959836A8F7A833E9A8F46A25E4`。
    - [x] Attempt007 actual generated pre-latch definitions 已通过此前修复路径，但在 repository gate 以 exit 71 / `Current Git authority drifted.` fail-closed。旧 verifier 将 Git index `TREE` extension 与自身按 ordinal sibling 顺序重建的 cache-tree 做 byte-for-byte 比较，因合法 sibling 顺序不同而错误拒绝；outer 从未调用，latch 不存在。
    - [x] Attempt007 cache `TREE` sibling-order 根因已修复并通过 reviewer：commit `01c2a8bfd1e9e4835511f68f8b9cedd85e48ae16`、tree `3e528578184c92c0a689cad137b68c98a9097531`；RED 复现合法 reordered sibling rejection，GREEN 为 FLB 243/313、focused bridge 1/1、Release build 0 warning/0 error、format/diff 通过，真实 private repository verifier 返回 `trackedClean=true`；reviewer `PASS`，`BLOCKER/HIGH/MEDIUM/LOW = 0/0/0/0`。
    - [x] RAB-6、FLB-7 与 Attempt007 仅保留为失败证据并永久不可复用；没有 UAC、RunAs、WAL、state、journal、anchor、Program Files、ProgramData、service、process、pipe 或其他系统 mutation。
    - [x] 已从 clean commit `80f92bbecf44f4162567c0ddab67d0074353d224`、tree `ed0327d51159137940c6283a68a1527ec87cc725` 生成并通过 public validator 验证 RAB-7 `install-wal-rollback-7` / contract `FSL-CP10-INSTALL-WAL-ROLLBACK-RECOVERY-7` 与 FLB-8 `install-wal-rollback-launch-observer-8` / contract `FSL-CP10-INSTALL-WAL-ROLLBACK-LAUNCH-OBSERVER-8`。RAB canonical/wrapper/contract SHA-256 为 `9AA5EA45A1D677C42FAB0DDCD1E7DAB82311446AED1CB5B40E4E568762002505`、`1A8A61CB7D7FD10853237349B0653907FA43D206C062960D31A36E4512161209`、`F8A2DD8D1C30DFAD7186CAA902311EF5BCFCA8ED39159068779FB8E59FF58817`；FLB canonical/outer/observer/contract SHA-256 为 `0EB2AD9B8C62D3D8FEC70B99F69EB7BBCA1BE95DD231BA2435578B36EACCF99E`、`19121D260386E833941B53CDF8754F589911486EF96AF9FA07EFA86537D5348E`、`F38011CF9267B6214DAE4EF3488481F69713583B788A9E1D8DA133878694F302`、`57EE78670A58A07BB6BE396E80EE240EACF94FED34AA2FF35F3FDE38DEE59C60`。
    - [x] Attempt008 actual generated definitions-only pre-latch diagnostic 在任何 observer top-level、outer、latch 或 RunAs 前以 exit 74 / `Release fingerprint drifted.` fail-closed；outer 从未执行，`launch-attempt.jsonl` 从未创建。RAB 冻结的 22-file canonical record fingerprint 为 `F5ECE1435052A83B91DB241D5F562BE4059BD8782F884B49449963FC5A146C16`，旧 observer 的 `Name|Length|Hash` ad-hoc 文本算法实际产生 `B5244B4595BB0DC10F9F8B5413BB3829149AF673E5D20B24B2C2B581ABAAA90E`。
    - [x] Attempt008 release fingerprint 根因已修复并通过 reviewer：commit `1a2f37ee827e32a28940773d96ea61615e51263c`、tree `af6c93bcc4e27c70762bc58108c9a65aff8e959c`；generated observer 现按 RAB 冻结顺序重建 exact `{path,length,sha256}` records，并使用相同 canonical JSON、LF、strict UTF-8 no BOM 与 uppercase SHA-256 规则。exact set/path/length/hash/fingerprint drift 均固定 exit 74；FLB 248/318、RAB 222/309、focused bridges 2/2、Release build 0 warning/0 error、三个 PowerShell 文件 parser errors 0、format/diff 与三文件边界通过。
    - [x] 独立 reviewer 返回 `PASS`，`BLOCKER/HIGH/MEDIUM/LOW = 0/0/0/0`，无 findings。RAB-7、FLB-8 与 Attempt008 仅保留为失败证据并永久不可复用；repair 未生成 RAB-8、FLB-9 或 Attempt009，未执行 UAC、RunAs、WAL、state、journal、anchor、安装、服务、重启或其他系统 mutation。
    - [ ] 在新的文档 commit-freeze 后生成并验证 RAB-8 `install-wal-rollback-8` / contract `FSL-CP10-INSTALL-WAL-ROLLBACK-RECOVERY-8`，以及 FLB-9 `install-wal-rollback-launch-observer-9` / contract `FSL-CP10-INSTALL-WAL-ROLLBACK-LAUNCH-OBSERVER-9`；新的唯一 attempt 为 `CP10-IWRR-LAUNCH-ATTEMPT-009`。
    - [ ] 仅在 RAB-8、FLB-9 和完整 actual generated pre-latch 重新通过后，使用全新 Attempt009 执行唯一 one-shot observer/UAC；不得 retry、fallback、第二次 Install 或复用旧 latch/attempt。
